...
for g=1:m,
    if mod(g,1) == 0,
        ...
        if liveFLAG == 1,
            [imu, acc1, acc2, acc3, acc4, joints] = readInput(u,0);
        else
            [imu, acc1, acc2, acc3, acc4, joints] = readInput(u,g);
        end
%         imu(1,1) = -1*imu(1,1);
%         %imu(1,2) = -1*imu(1,2);
%         R0G = [cos(imu(1,1)) 0 -sin(imu(1,1));...
%             sin(imu(1,1))*sin(imu(1,2)) cos(imu(1,2)) cos(imu(1,1))*sin(imu(1,2));...
%             sin(imu(1,1))*cos(imu(1,2)) -sin(imu(1,2)) cos(imu(1,1))*cos(imu(1,2))];
        ...
        acc2_Norm = Normalize(acc2(1,1:3));
        acc3_Norm = Normalize(acc3(1,1:3));
        acc4_Norm = Normalize(acc4(1,1:3));

        setGlobalAcc1(acc1_Norm);
        setGlobalAcc2(acc2_Norm);
        setGlobalAcc3(acc3_Norm);
        setGlobalAcc4(acc4_Norm);
        ...
        options2 = optimoptions('fsolve','Display','iter');
        options3 = optimset('Algorithm','levenberg-marquardt','Display','iter');

        % Determine Joints One and Two
        %x0 =[p_j1,p_j2];
        %x0 =[1.5,1.5];
        x0 =[1.5,1.0];
        [x,fval] = fsolve(@myFunction12mod,x0,options3);
        j1 = x(1,1);
        j2 = x(1,2);
        setGlobalJ1(j1);
        setGlobalJ2(j2);
...
% setGlobalJ2(joints(1,2));
        ...
        [x,fval] = fsolve(@myFunction34mod,x0,options3);
        j3 = x(1,1);
        j4 = x(1,2);
         setGlobalJ3(j3);
         setGlobalJ4(j4);
% setGlobalJ3(joints(1,3));
% setGlobalJ4(joints(1,4));

        % Determine Joints Five and Six
        %x0 =[p_j5,p_j6];
        x0 =[1.5,0.9];
        [x,fval] = fsolve(@myFunction56,x0,options3);  
        j5 = x(1,1);
        j6 = x(1,2);
         setGlobalJ5(j5);
         setGlobalJ6(j6);
% setGlobalJ5(joints(1,5));
% setGlobalJ6(joints(1,6));

        % Determine Joint Seven
        %x0 =[p_j7];
        x0 =[1.5];
        [x,fval] = fsolve(@myFunction7,x0,options1);
        j7 = x(1,1);
        ...
        % Plot Data  
        if j1>3.2 || j2>3.2 || j3>3.2 || j4>3.2 || j5>3.2 || j6>3.2 || j7>3.2 || j1<-3.2 || j2<-3.2 || j3<-3.2 || j4<-3.2 || j5<-3.2 || j6<-3.2 || j7<-3.2 ,
            prev = listIndex-1;
%             listIMUKnown(listIndex,1:3) = listIMUKnown(prev,1:3);
            ...
            %listJointsKnown(listIndex,6) = 1.04-(joints(1,6)-1.04);
            ...
            listJointsComputed(listIndex,7) = double(j7);

            
%             listIMUKnown(listIndex,1:3) = imu(1,1:3);
